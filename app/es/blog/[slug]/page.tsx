...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  const title = blogTitle(params.slug, 'es', article.title)
  const description = blogExcerpt(params.slug, 'es', article.excerpt)
  return {
    title,
    description,
    alternates: {
      canonical: `https://bloomfocus.org/es/blog/${params.slug}`,
    ...
    robots: hasTranslation(params.slug, 'es') ? undefined : { index: false, follow: true },
  }
}
...
export default function BlogArticlePage({ params }: { params: { slug: string } }) {
  const article = articles.find(a => a.slug === params.slug)
  const title = article ? blogTitle(params.slug, 'es', article.title) : ''
  const description = article ? blogExcerpt(params.slug, 'es', article.excerpt) : ''
  const jsonLd = article ? {
    ...
    mainEntityOfPage: { '@type': 'WebPage', '@id': `https://bloomfocus.org/es/blog/${params.slug}` },
    url: `https://bloomfocus.org/es/blog/${params.slug}`,
    // Article schema wants an image; point at this locale's generated
    // Open Graph card so the rich result has one.
    image: {
      '@type': 'ImageObject',
      url: `https://bloomfocus.org/es/blog/${params.slug}/opengraph-image`,
      width: 1200,
      height: 630,
    },
    inLanguage: servedLanguage(params.slug, 'es'),
  } : null
  ...
  const breadcrumbLd = article ? {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://bloomfocus.org/es' },
      { '@type': 'ListItem', position: 2, name: 'Blog', item: 'https://bloomfocus.org/es/blog' },
      { '@type': 'ListItem', position: 3, name: article.title, item: `https://bloomfocus.org/es/blog/${params.slug}` },
    ],
  } : null
  return (
    <>
      ...
      <BlogArticle lang="es" slug={params.slug} />
    </>
  )
}
