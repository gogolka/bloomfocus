...
const products = [
  {
    title: 'Dopamine Menu',
    desc: 'A reward system your ADHD brain will actually use. Pick a treat, no guilt.',
  ...
  },
  {
    ...
    desc: 'Energy-based time blocks. Brain dump space. No shame, no pressure.',
    tag: 'Printable · PDF',
    color: '#FFD6C4',
    borderColor: '#FFBFA8',
    emoji: '📋',
  ...
  {
    title: 'Habit Tracker',
    desc: 'Track what matters, not everything. Celebrate every single win.',
    tag: 'Undated · A4 & Letter',
    color: '#D4E8D4',
    borderColor: '#B8D4B8',
    emoji: '🌱',
  },
  {
    ...
    desc: 'Empty your head onto paper. No judgment, no structure needed.',
    tag: 'Instant download',
    color: '#D4EEFF',
    borderColor: '#D4EEFF',
    emoji: '🧠',
      ...
      <section style={{ position: 'relative', overflow: 'hidden', padding: '80px 24px 100px', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 680, margin: '0 auto' }}>
          ...
          <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(36px, 6vw, 64px)', color: '#2D2926', lineHeight: 1.1, letterSpacing: '-1.5px', marginBottom: 20 }}>
            Planning that works with<br />
            your brain,{' '}
            <em style={{ color: '#B8A4E8' }}>not against it.</em>
          </h1>

          <p style={{ fontSize: 17, color: '#6B5F58', lineHeight: 1.7, maxWidth: 500, margin: '0 auto 36px' }}>
            Tools designed for neurodivergent brains — no shame, no pressure, no "just try harder."
            Finally, a planner your ADHD brain actually wants to use.
          </p>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/shop" style={{
              ...
              boxShadow: '0 4px 20px rgba(184,164,232,0.35)',
              display: 'inline-block',
            }}>
              Shop the toolkit ✨
            </Link>
            <Link href="/blog" style={{
              textDecoration: 'none', background: 'transparent', color: '#6B5F58',
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap', marginTop: 32 }}>
            {['Neurodivergent-first', 'No shame, no pressure', 'Instant download', 'Warm & structured'].map((t, i) => (
              <div key={i} style={{
                background: ['#E8DEFF', '#FFD6C4', '#D4E8D4', '#D4EEFF'][i],
                border: `1.5px solid ${['#D4C5F9', '#FFBFA8', '#B8D4B8', '#D4EEFF'][i]}`,
                borderRadius: 100, padding: '5px 14px', fontSize: 12, color: '#2D2926',
              }}>{t}</div>
            ))}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(26px, 4vw, 38px)', color: '#2D2926', lineHeight: 1.2 }}>
              ...
              <em style={{ color: '#B8A4E8' }}>It just needs a different system.</em>
            </h2>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 14 }}>
            {pains.map((pain, i) => (
              <div key={i} style={{
                ...
                boxShadow: '0 2px 12px rgba(45,41,38,0.04)',
              }}>
          ...
          <div style={{ textAlign: 'center', marginTop: 40 }}>
            <p style={{ fontFamily: 'Georgia, serif', fontSize: 18, color: '#2D2926', fontStyle: 'italic', lineHeight: 1.6 }}>
              "We're the friend who also has ADHD — who gets it, doesn't judge,<br />
              and has actually found things that help."
            </p>
          </div>
        </div>
      </section>
      ...
      <section id="shop" style={{ padding: '80px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 20 }}>
            {products.map((p, i) => (
              <div key={i} style={{
                background: p.color, border: `1.5px solid ${p.borderColor}`,
                borderRadius: 20, padding: '28px 24px',
                transition: 'transform 0.2s, box-shadow 0.2s',
                cursor: 'pointer',
              }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(-4px)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 8px 32px rgba(45,41,38,0.1)'; }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(0)'; (e.currentTarget as HTMLElement).style.boxShadow = 'none'; }}
              >
                <div style={{ fontSize: 40, marginBottom: 16 }}>{p.emoji}</div>
                <div style={{ background: 'rgba(255,255,255,0.6)', borderRadius: 100, padding: '3px 12px', fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#6B5F58', display: 'inline-block', marginBottom: 12 }}>
                  {p.tag}
                </div>
                ...
                <Link href="/shop" style={{
                  textDecoration: 'none', background: '#2D2926', color: 'white',
                  padding: '10px 20px', borderRadius: 100, fontSize: 12, fontWeight: 600, display: 'inline-block',
                }}>View product →</Link>
              </div>
            ))}
          </div>
          ...
          <div style={{ textAlign: 'center', marginTop: 40 }}>
            <Link href="/shop" style={{
              textDecoration: 'none', background: 'transparent', color: '#2D2926',
              padding: '13px 28px', borderRadius: 100, fontSize: 14, fontWeight: 600,
              border: '1.5px solid rgba(45,41,38,0.2)', display: 'inline-block',
            }}>
              See all products →
            </Link>
          </div>
            ...
            {blogPreviews.map((post, i) => (
              <Link key={i} href={`/blog/${post.slug}`} style={{ textDecoration: 'none' }}>
                <div style={{
                  ...
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(-4px)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 8px 24px rgba(45,41,38,0.08)'; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(0)'; (e.currentTarget as HTMLElement).style.boxShadow = 'none'; }}
                >
                  <div style={{ background: '#E8DEFF', borderRadius: 100, padding: '3px 12px', fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#7B5FCC', display: 'inline-block', marginBottom: 16 }}>
                    {post.tag}
                  </div>
      ...
      <section style={{ padding: '80px 24px', background: '#2D2926', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 600, margin: '0 auto' }}>
          <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(28px, 4vw, 42px)', color: '#FEFCFA', lineHeight: 1.2, marginBottom: 16 }}>
            One small step.<br />
            <em style={{ color: '#B8A4E8' }}>That's enough.</em>
          </h2>
          ...
          <Link href="/shop" style={{
            ...
            display: 'inline-block', boxShadow: '0 4px 24px rgba(184,164,232,0.4)',
          }}>
