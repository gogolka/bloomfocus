    ...
    <div>
      ...
      <div style={{ background: '#FEFCFA', border: '1px solid rgba(45,41,38,0.08)', borderRadius: 16, padding: 14, marginBottom: 20 }}>
        <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
          <input value={newTask} onChange={e => setNewTask(e.target.value)} onKeyDown={e => e.key === 'Enter' && addTask()} placeholder={tr.whatsThePlan} style={{ flex: 1, border: '1.5px solid rgba(45,41,38,0.12)', borderRadius: 12, padding: '11px 14px', fontSize: 14, color: '#2D2926', background: 'white', outline: 'none', fontFamily: "'DM Sans', sans-serif" }} />
          <button onClick={addTask} style={{ background: '#B8A4E8', color: 'white', border: 'none', borderRadius: 12, padding: '11px 18px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>{tr.addTask}</button>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#9B8F88' }}>
            <span>🏷</span>
            <input value={newTagInput} onChange={e => setNewTagInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' || e.key === ',') { e.preventDefault(); addTag() } }} placeholder={tr.addTag} style={{ border: '1px solid rgba(45,41,38,0.12)', borderRadius: 8, padding: '6px 10px', fontSize: 12, color: '#2D2926', background: 'white', outline: 'none', fontFamily: "'DM Sans', sans-serif", width: 130 }} />
          </div>
      ...
      {tasks.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '48px 0', color: '#9B8F88' }}>
          ...
          <div style={{ fontSize: 13 }}>{tr.noTasksYet}</div>
        </div>
      ) : (
                ...
                {isExp && (
                  <div style={{ background: '#FFF8F0', borderTop: '1px solid rgba(45,41,38,0.06)', padding: '12px 16px' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                      <input value={newStep} onChange={e => setNewStep(e.target.value)} onKeyDown={e => e.key === 'Enter' && addStep(task.id, task)} placeholder={tr.addMicroStep} style={{ flex: 1, border: '1px solid rgba(45,41,38,0.1)', borderRadius: 8, padding: '7px 10px', fontSize: 13, background: 'white', outline: 'none', fontFamily: "'DM Sans', sans-serif" }} />
      ...
      {view === 'done' && (
        <div>
          ...
          {!doneLoading && (doneTasks || []).length === 0 && (
            <div style={{ textAlign: 'center', padding: '48px 0', color: '#9B8F88' }}>
              <div style={{ fontSize: 40, marginBottom: 12 }}>🌙</div>
              <div style={{ fontFamily: 'Georgia, serif', fontSize: 16, color: '#2D2926', marginBottom: 4 }}>{tr.completedTasks}</div>
              <div style={{ fontSize: 13 }}>{tr.finishedTasks}</div>
            </div>
          )}
          {!doneLoading && (doneTasks || []).length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {(doneTasks || []).map(task => (
                <div key={task.id} style={{ background: '#FEFCFA', border: '1px solid rgba(45,41,38,0.08)', borderRadius: 16, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <button onClick={() => reopenTask(task)} title="Reopen" style={{ background: 'none', border: '1px solid rgba(123,95,204,0.25)', color: '#7B5FCC', borderRadius: 100, padding: '5px 12px', fontSize: 11, fontWeight: 600, cursor: 'pointer', flexShrink: 0, fontFamily: "'DM Sans', sans-serif" }}>{tr.reopen}</button>
                </div>
              ))}
            </div>
          )}
